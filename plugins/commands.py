...
async def start(client, message):
    ...
    if mc.startswith('verify'):
        ...
        if verify_status["link"] == "":
            ...
            reply_markup = InlineKeyboardMarkup(btn)
        await message.reply(f"<b>Now You are successfully verified For Today ✓\n\nNow You have Unlimited Access For All Movies Till </b>: {get_readable_time(VERIFY_EXPIRE)}", reply_markup=reply_markup, protect_content=True)
        return
    ...
    if IS_VERIFY and not verify_status['is_verified']:
        ...
        await message.reply(f"<b>Thanks To Use Me 🤝\n\n📌 ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴠᴇʀɪꜰɪᴇᴅ ᴛᴏᴅᴀʏ, ᴘʟᴇᴀꜱᴇ ᴠᴇʀɪꜰʏ ᴀɴᴅ ɢᴇᴛ ᴜɴʟɪᴍɪᴛᴇᴅ ꜰɪʟᴇ ᴀᴄᴄᴇꜱꜱ ꜰᴏʀ ᴛᴏᴅᴀʏ\n\n#Verification ✓</b>", reply_markup=InlineKeyboardMarkup(btn), protect_content=True)
        return
