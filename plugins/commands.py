...
async def start(client, message):
    ...
    if mc.startswith('verify'):
        ...
        if verify_status["link"] == "":
            ...
            reply_markup = InlineKeyboardMarkup(btn)
        await message.reply(f"<b>Now You are successfully verified For Today ✓\n\nNow You have Unlimited Access For All Movies Till: {get_readable_time(VERIFY_EXPIRE)}", reply_markup=reply_markup, protect_content=True)</b>
        return
